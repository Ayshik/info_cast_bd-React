...
const Package_details=()=>{
  ...
  useEffect(()=>{
      axios.get("http://localhost:8000/api/Modarator_info")
      .then(resp=>{
...
const Delete = (e,id) => {
  e.preventDefault();
    ...
    axios.delete(`/api/Delete_mod/${id}`)
    ...
    toast.error('Deleted', {
      ...
      closeOnClick: true,
       ...
       window.location.href ='/Modarator_info';
              ...
              <div className="card my-4">
                <div className="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                  <div className="bg-gradient-primary shadow-primary border-radius-lg pt-4 pb-3">
                    <h6 className="text-white text-capitalize ps-3">Moderator Detais</h6>
                  </div>
                </div>
                ...
                products.map(p=>(
                        <tr key={p.id}>
                          ...
                          <td className="align-middle text-center text-sm">
                            <span className="badge badge-sm bg-gradient-success">{p.phone}</span>
                          </td>
